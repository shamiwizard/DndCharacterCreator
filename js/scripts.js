...
function SelectBackstory(element) {                                                          // Function for confirmation Backstory              
    ...
    btnSelected.onclick = function () {
        if (confirm('Ви впевені що хочити вибрати предісторію ' + this.value)) {
            ...
            openSgsChrctrstc.querySelector('button').onclick = function () {
                ChoiceCharacteristics(openSgsChrctrstc);
            };
            
        }
    }
}
function checkboxMax(numberMax, whereFind) {                                                  // function for max checkbox 
...
function ChoiceCharacteristics(element) {
    var areaButtons = element.querySelectorAll('div:only-of-type');
    var notValidate = true;
    for (let i = 0; i < areaButtons.length; i++) {
        var buttonsCheck = areaButtons[i].querySelectorAll('input:checked');
        if (buttonsCheck.length <= 0) {
            areaButtons[i].classList.add('error');
            notValidate= false
        }else areaButtons[i].classList.remove('error');
    }
    if(notValidate)
    {
       if(confirm('Ви впевені що хочити вибрати предісторію'))
       {
         document.querySelector('.h-backstory').classList.remove('expected');
         document.querySelector('.h-backstory').classList.add('selected');
       }; 
    }
}